...
While you are signed in, use the **bug icon** at the bottom-right of the screen (it may appear faint until you point at it). The tooltip reads **Report an issue · suggest an idea**.

### How to send a report

1. Click the bug icon. A panel opens titled **Tell us what went wrong**.
2. Describe what happened in **What happened?** (required). Do not type student names in the box—use a student ID from the list when needed.
3. Optionally choose **How is this affecting you?** — one of four chips, including **Suggest an improvement** for ideas that are not broken behavior.
4. A screenshot of the page is captured automatically. You can remove it before sending if you prefer.
5. To attach a specific row, use **Point at the spot on this page**, then click the row or cell.
6. To browse the page for a student ID, use **Look on the page**. The panel collapses to **Report in progress — Continue** so you can scroll or change filters, then tap **Continue**.
7. Optionally enter an **8-digit Student ID** from the Students list (not the student’s name).
8. Tap **Send**. You briefly see a confirmation with a reference such as **#HF-1042**, and you receive **one** email receipt at your account address. hFlow does not send email when status changes later.

### Owners: reported issues list

School **owners** open **Configuration → Reported issues** to see what staff submitted, open a report for the screenshot and details, and **withdraw** a mistaken report if needed. Withdrawn reports show status **Withdrawn**.
