...
### I do not see a notice a colleague mentioned

Notices can be limited to **teachers**, **admins**, or **owners**. You only see notices for your role, and some notices go only to one school. If you joined the school after the notice was published, look in the bell list; you may not get the top banner. See [Notifications](/hflow/notifications).

### How do I dismiss the amber bar at the top?

Click **X** on the bar. That hides it for you and marks the notice as read. It remains under **Read** in the bell. If the bar shows **Message 1 of n**, another notice appears after you dismiss this one.
...
**Common blockers:** editing an imported assessment in the gradebook; adding assessments for newly imported students; changing a student’s class after a roster CSV updated classes; imports from before the undo feature (no journal).

### Class errors on import

If a grade already has classes, provide a `class` value for each row. If the grade has no classes, leave `class` blank. Typing `5a` or `5b` in the CSV creates those classes automatically. You do not set them up elsewhere first.
